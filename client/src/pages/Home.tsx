  ...
  return (
    <div className="min-h-screen bg-background text-foreground overflow-x-hidden relative">
        ...
        <div className="container relative z-20 px-4 text-center">
          ...
          >
            ...
            <h1 className="text-4xl sm:text-5xl md:text-7xl lg:text-9xl font-display font-black text-white uppercase tracking-tighter mb-6 text-shadow-glow leading-none">
              {t('hero.title1')} <br /> 
              <span className="text-transparent bg-clip-text bg-gradient-to-r from-brand-blue to-white">{t('hero.title2')}</span>
            </h1>
        ...
        <div className="container mx-auto px-4 relative z-10">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 text-center">
            {[
              ...
              { icon: Gavel, label: t('features.quality_guarantee'), desc: t('features.quality_guarantee_desc') },
              { icon: Box, label: t('features.large_stock'), desc: t('features.large_stock_desc') }
            ].map((feature, idx) => (
              <motion.div
                key={idx}
              ...
              >
                ...
                <h3 className="text-xl font-bold text-foreground mb-2">{feature.label}</h3>
      ...
      <section className="py-24 bg-background relative">
        <div className="container mx-auto px-4">
          <div className="flex flex-col md:flex-row justify-between items-end mb-12 gap-6">
            <div>
              <span className="text-brand-orange font-mono uppercase tracking-widest text-sm font-bold">{t('home.featured_outline')}</span>
              <h2 className="text-4xl md:text-5xl font-display text-foreground mt-2">
                {t('home.featured_title')} <span className="text-outline-blue">{t('home.featured_outline')}</span>
              </h2>
            </div>
            <Link href="/products">
              <span className="text-brand-blue hover:text-white transition-colors cursor-pointer flex items-center gap-2 uppercase tracking-wider font-bold text-sm group">
                {t('home.view_all')} <ArrowRight className="w-4 h-4 group-hover:translate-x-1 transition-transform" />
              </span>
            </Link>
          </div>
      ...
      <section className="py-24 bg-card border-t border-white/5">
        <div className="container mx-auto px-4">
          <h2 className="text-4xl md:text-5xl font-display text-foreground mb-12 text-center uppercase">
            {t('home.categories_title')} <span className="text-brand-orange">{t('home.categories_outline')}</span>
          </h2>
      ...
      <section className="py-24 relative overflow-hidden">
        ...
        <div className="container mx-auto px-4 relative z-10 text-center">
          <h2 className="text-4xl md:text-6xl font-display font-bold text-foreground mb-6 uppercase">
            {t('home.ready_to_build').split('?')[0]} <span className="text-brand-orange">?</span>
          </h2>
